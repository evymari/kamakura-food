...
function showReceipt(cartItems) {
  ...
  const receiptProductContainer = document.getElementById("receipt-product");
    ...
    const receiptItems = JSON.parse(JSON.stringify(cartItems))

    while ( receiptProductContainer.firstChild )
    {
        receiptProductContainer.removeChild(receiptProductContainer.firstChild)
    }
  ...
  document.getElementById("receipt-total").innerText = `Total: ${total.toFixed(
    2
  )} €`;
